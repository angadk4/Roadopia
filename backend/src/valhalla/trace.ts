 ...
 *
 * Verified against the local 3.7.0 container: request takes the decoded shape
 * as {lat,lon} points, `shape_match: 'walk_or_snap'` (edge-walk first — our
 * shapes ARE Valhalla routes — snap fallback); response edges carry
 * UNPREFIXED `road_class` + `length` in the response's `units` (default
 * kilometres); partial first/last edges report their FULL length plus
...
/** Map a route geometry to its per-edge road classes. */
