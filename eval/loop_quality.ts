...
async function evaluateBrief(db: Client, brief: string): Promise<BriefReport> {
  ...
  let scope = await buildScope(VALHALLA, { origin, shape: 'loop', durationS });
  let retrieved = await retrieveCandidates(db, scope, {
  ...
  if (candidates.length < K_PRESENT_DEFAULT * 2) {
    scope = await buildScope(VALHALLA, {
      origin,
      shape: 'loop',
      durationS: Math.round(durationS * 1.3),
    });
    retrieved = await retrieveCandidates(db, scope, {
      stopTypes: constraints.stops.map((s) => s.type),
      thetaCurvy: 0.4,
    });
    candidates = generateLoopCandidates(origin, retrieved.segments, retrieved.spots, {
      anchorSpots: retrieved.spots.length > 0,
      durationS,
    });
    notes.push('ladder-assisted (τ×1.3, θ 0.4)');
  }
